...
class ManageController extends Controller{
    ...
    public function basic_update(Request $request){
        ...
        if($request->hasfile('flogo')){
            ...
            $flogoName = 'flogo'.time().'.'.$flogo->getClientOriginalExtension(); 
            $manager = new ImageManager(new Driver());
        ...
        if($update){
            ...
            Session::flash('errors','Opps! Operation failed');
            return redirect('dashboard/manage/basic');
        }
    }
    ...
    public function social_update(Request $request){
        ...
        if($update){
            Session::flash('success','Successfully update social information.');
